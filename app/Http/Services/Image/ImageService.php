...
{
    public function storeImage($request, $field, $folderName, $method = 'index')
    {
        if ($request->hasFile($field)) {
            $this->setExclusiveDirectory('images' . DIRECTORY_SEPARATOR . $folderName);
            if ($method == 'index') {
                $result = $this->createIndexAndSave($request->file($field));
                $inputs[$field]['currentImage'] = $request['currentImage'];
            } else {
                $result = $this->save($request->file($field));
            }
            if ($result === false) {
